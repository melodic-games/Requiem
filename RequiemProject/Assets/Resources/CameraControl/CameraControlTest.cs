...
public class CameraControlTest : MonoBehaviour {
    ...
    public static CameraControlTest instance;
    ...
    [Header("Camera Up Direction Reset")]
    ...
    void Update()
    {
        ...
        if (Time.deltaTime > 0)// && Time.timeSinceLevelLoad > 5)
        {
            ...
            //Set Camera FOV
            {
                ...
                thisCamera.fieldOfView = Mathf.Lerp(thisCamera.fieldOfView, camTargetFov + skyBonus, Time.deltaTime * 10);

                fovRatio =  Mathf.Lerp(thisCamera.fieldOfView, camTargetFov, Time.deltaTime * 10) / 60;
            ...
            //Camera Lean            
            Vector3 lean = cameraVelocity * .01f;
            ...
            if (upReset)
            {
                ...
                float disable = 1 - Mathf.Clamp01(2 * Mathf.Abs(Vector3.Dot(targetRotation * Vector3.forward, cameraUpVector)));
                targetRotation = Quaternion.Lerp(targetRotation, Quaternion.LookRotation(targetRotation * Vector3.forward, (cameraUpVector + lean).normalized), Time.unscaledDeltaTime * disable * upResetLerpSpeed);
            }
