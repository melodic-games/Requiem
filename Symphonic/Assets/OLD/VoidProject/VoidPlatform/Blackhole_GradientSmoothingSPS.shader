...
Shader "MAG/Blackhole_GradientSmoothingSPS" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 viewDirection = normalize(centerEye - i.posWorld.xyz);
                float3 normalDirection = i.normalDir;
                float2 sceneUVs = (i.projPos.xy / i.projPos.w);
                float4 sceneColor = tex2D(_GrabTexture, UnityStereoTransformScreenSpaceTex(sceneUVs).xy);
////// Lighting:
////// Emissive:
