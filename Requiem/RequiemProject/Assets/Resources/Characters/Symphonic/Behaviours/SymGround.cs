            ...
            if (owner.grounded)
            {
                ...
                //Character faces the move direction, or velocity direction, or the current direction                                      
                {
                    ...
                    if (owner.moveDirectionMag > .1f && !owner.muteInput)
                        owner.targetForward = owner.moveDirection;                        
                    else if (owner.rbVelocityMagnitude > 1 && owner.focusInput && !owner.muteInput)
                        owner.targetForward = Camera.main.transform.forward;                                                    
                    else
                        owner.targetForward = owner.transform.forward;

                    //Crush the forward vector to make it orthogonal to upwards vector                    
                    owner.targetForward = (owner.targetForward - (upwards * Vector3.Dot(owner.targetForward, upwards))).normalized;     
                    ...
                    owner.transform.rotation = Quaternion.Slerp(owner.transform.rotation, Quaternion.LookRotation(owner.targetForward, upwards), Time.deltaTime * rotationSpeedScale);
                }     
                ...
                //turnScale Calulation   
                {
                    ...
                    //find Increment value
                    {
                        ...
                        float characterFacingTargetForward = Vector3.Dot(owner.targetForward, owner.transform.forward);
        ...
        if (owner.grounded)
        {
            ...
            if (!owner.muteInput)
            {
                if (owner.boostBuffer <= owner.boostBufferMax)
                {
                    ...
                    {
                        owner.rb.velocity = owner.moveDirectionRaw.normalized * owner.boostSpeed;
