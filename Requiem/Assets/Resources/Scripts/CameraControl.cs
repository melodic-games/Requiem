...
public class CameraControl : MonoBehaviour {
    ...
    private float disableTracking = 1;
    [Range(0,20)]
    ...
    void Update()
    {
        ...
        if (upReset)
        {
            if(gM != null)
            {
                ...
                gravity = Vector3.Lerp(-gM.ReturnGravity(transform), manualUpVector, manualUpVectorScaler);
                upVector = Vector3.Lerp(manualUpVector,gravity.normalized, gravity.magnitude);
            }
            else
            {
        ...
        if (followingTarget)       
        {
            ...
            if (targets[followIndexPosition] != null)
            {
                ...
                //set camera position                                       
                {
                    float disable = Mathf.Clamp01((Time.time * Time.time) / 10);
